...
보관 기간은 7·30·90일 또는 무기한입니다. 기간에 따른 실제 삭제는 기억이 켜진 상태로 패널이 열릴 때 수행하며, 검색할 때도 만료된 기록을 제외합니다. 제외 도메인은 `example.com`처럼 입력합니다. `mail.example.com` 등 하위 도메인의 신규 저장과 기존 기록 삭제에도 적용됩니다.

기억을 끄는 것과 기존 기억 삭제는 별개입니다. 제외·끄기·전체 삭제 시 이전에 시작된 임베딩은 저장 직전 정책 검사를 통과하지 못하므로 삭제한 기록을 다시 저장하지 않습니다. **앞으로도 저장하지 않으려면 기억을 끄고 기억 전체 삭제를 실행하세요.** 기억을 켠 채 전체 삭제만 하면 이후 새로 읽힌 페이지는 다시 저장할 수 있습니다. 검색은 모델·벡터 차원이 맞고 유사도 기준을 넘는 기록만 사용합니다.
...
| 입력 예산 초과 | 본문/화면 첨부를 분리하거나 질문을 나누세요. 시스템·도구·이미지까지 합산한 추정 입력이 컨텍스트의 70%를 넘으면 전송을 거부합니다. 필요하면 설정에서 컨텍스트를 늘리세요. |
