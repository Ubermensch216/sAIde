...
문서 기준: **2026-09-20 · 소스 버전 0.1.0**. 현재 배포 형태는 소스를 빌드해 설치하는 개발자 모드 확장입니다. 이 저장소에는 Chrome 웹스토어 공개 배포를 확인할 링크가 없습니다.
...
| 선택 영역만 질문 | 문단을 드래그 → **선택 영역 붙이기** → 그 부분에 대해 자유롭게 질문 | 해당 사이트 접근 허용 |
...
| 할 일·기한 뽑기 | `/조치` — 페이지의 할 일·기한·제출물을 원문 대조와 함께 정리 | 읽을 수 있는 본문 |
| 일정 관리 | `@일정 내일까지 보고서 제출`, `@일정 이번 주 보여줘` | 일정 탭 |
| PDF 본문 읽기 | 브라우저 PDF 뷰어로 열린 문서 요약·질문 | 해당 사이트 접근 허용 |
| 사용자 프리셋 | 반복해서 쓰는 지시를 `/명령어`로 등록 | 설정 화면 |

모델이 내놓는 답은 틀릴 수 있습니다. AI가 쓴 답변에는 `AI 생성 · 검토 필요` 표시가 붙고, 코드가 저장소를 읽어 만든 일정 목록에는 다른 표시가 붙습니다. 본문이 잘렸다는 표시가 있으면 읽힌 범위만 답변 근거가 됩니다. 브라우저 내부 화면, Shadow DOM·캔버스 중심 앱은 일반 HTML 본문처럼 읽거나 조작되지 않을 수 있습니다. PDF는 글자가 들어 있는 문서만 읽을 수 있으며, 스캔 이미지로 된 PDF는 글자가 나오지 않습니다.
...
| 위치 | 역할 |
|---|---|
| 상단 도구 모음 | 대화 제목, 대화 초기화, 대화 목록, 설정 |
| 탭 줄 | **AI**와 **일정**. 일정 탭에는 기한이 임박한 건수가 배지로 표시됩니다 |
| 현재 페이지 영역 | 대상 페이지 확인, 요약·핵심 3줄·번역 등 실행 |
| 페이지 첨부 표시 | 제목, 추출 방식, 대략의 토큰 수, 잘림 여부. ×로 분리 |
| 선택 영역 표시 | 드래그해 붙인 부분의 첫 줄, 대략의 토큰 수, 잘림 여부. ×로 분리 |
...
메시지에 마우스를 올리면 **복사**와 **삭제**가 나타납니다. 답변 아래의 **맞음·틀림**은 이 컴퓨터에만 기록되며, 누적 정확도는 설정 화면의 정확도 기록에서 볼 수 있습니다. 같은 버튼을 다시 누르면 취소됩니다.

대화는 로컬에 저장됩니다. 대화 목록에서 제목을 눌러 이름을 바꿀 수 있습니다. 탭이나 페이지 주소가 바뀌면 해당 문서의 대화로 전환되며 기존 페이지·화면 첨부는 분리됩니다. 목록에서 항목을 열면 새 창 팝업이 뜨는 사이트에서는 대화를 유지한 채 읽을 대상만 옮깁니다. 이전 대화는 목록에서 다시 열 수 있습니다. **첨부 본문과 스크린샷 자체는 대화 메시지처럼 영구 저장되지 않으므로**, 기록을 다시 열었을 때 페이지를 근거로 답하게 하려면 다시 첨부하세요. 대화 목록에는 최근 50건이 표시됩니다.
...
| `@일정`, `@schedule`, `@task` | `@일정 금요일까지 보고서 제출` |
| `/explain` | `/explain 설명할 문장` |
| `/polish` | `/polish 다듬을 문장` |

`/` 또는 `@`를 입력하면 자동완성 목록을 볼 수 있습니다. **접두 문자가 결과가 남는 곳을 가릅니다** — `/`로 시작하는 명령의 답은 AI 창 안에서 끝나고, `@`로 시작하는 명령은 결과가 다른 탭에 남습니다. `@` 명령을 실행해도 화면이 저절로 옮겨 가지 않으며, 답변 안의 링크를 눌러야 그 탭으로 이동합니다. 접두 문자를 잘못 쳐도 이름이 맞으면 찾아 줍니다.

브라우저가 PDF 뷰어로 연 문서도 요약할 수 있습니다. 화면에 뜬 PDF의 원본을 받아 글자를 뽑아 본문 앞에 붙입니다. 글꼴이 들어 있지 않은 한글 PDF도 읽히도록 CMap을 함께 배포합니다. 번역 대상 언어를 생략하면 한국어로, 원문이 한국어이면 영어로 번역하도록 요청합니다. 긴 본문은 읽기 예산에서 잘릴 수 있고 번역 결과도 컨텍스트 한도로 짧아질 수 있습니다. 분량을 나누거나 설정의 컨텍스트를 조정하세요.

선택한 문장만 처리하려면 웹페이지에서 문장을 선택한 후 우클릭하여 sAIde의 **번역·설명·다듬기** 메뉴를 사용합니다. 우클릭 메뉴 언어는 Chrome 언어를 따릅니다.

### 선택 영역에 대해서만 질문하기

페이지의 한 부분에 대해서만 묻고 싶을 때 씁니다.

1. 웹페이지에서 궁금한 문단을 **드래그**합니다.
2. 입력창 위의 **선택 영역 붙이기**를 누릅니다. (또는 드래그한 채로 우클릭 → **선택 영역으로 sAIde에 묻기**)
3. 고른 부분이 칩으로 붙습니다. 무엇이 붙었는지 첫 줄로 확인할 수 있습니다.
4. “이 부분을 쉽게 설명해줘”처럼 **자유롭게** 물어봅니다.

페이지 본문을 함께 붙여 두면 두 가지가 나란히 전달됩니다 — 본문은 앞뒤 맥락으로, 고른 부분은 질문이 가리키는 곳으로 쓰입니다. 고른 곳을 바꿔 가며 계속 물어볼 수 있고, 그때 이전 문답은 그대로 남습니다.

드래그한 뒤 사이드패널을 눌러도 페이지의 선택은 유지되므로 순서는 상관없습니다. 선택된 곳이 없으면 페이지 전체로 대신하지 않고 먼저 드래그하라고 알립니다 — 고르지 않은 본문을 읽는 대기시간을 대신 물리지 않기 위해서입니다. 다른 출처(cross-origin) 프레임 안의 선택은 브라우저가 막아 읽지 못합니다. 아주 긴 선택은 읽기 예산에서 잘릴 수 있으며 그때는 잘린 비율을 표시합니다.

고른 부분만 보내므로 페이지 전체를 붙일 때보다 첫 응답이 빠릅니다. 본문 2,000토큰이 약 15초인 데 비해, 한두 문단(약 300토큰)은 실측 프리필 131 tok/s 기준 약 2~3초입니다.

YouTube는 자막 추출을 먼저 시도하고 실패하면 읽을 수 있는 페이지 텍스트로 대체합니다. 자막이 없거나 접근할 수 없는 영상의 전체 내용을 이해했다고 가정하지 마세요.

## 할 일·기한과 일정 탭

`/조치`를 실행하면 붙여 둔 본문에서 **요지·할 일·제출물·기한·문의처**를 JSON 스키마로 뽑습니다. 모델이 자유 형식으로 쓰게 두면 항목을 빠뜨리거나 날짜를 바꿔 쓰기 때문입니다. 화면에 내놓기 전에 **코드가 원문과 대조**해 `원문 확인` 또는 `원문에서 찾지 못함`을 붙이고, 모델이 빠뜨린 “…까지” 기한은 코드가 원문에서 직접 찾아 따로 표시합니다. 확인하지 못한 값도 지우지 않습니다 — 판단은 사용자가 합니다.

답변 아래에는 **일정으로 등록** 카드가 붙습니다. 근거를 확인한 후보만 기본으로 체크되며, **체크한 것만** 등록됩니다. 자동으로 등록되는 경로는 없습니다. 같은 페이지를 다시 분석해도 같은 할 일이 두 번 쌓이지 않고, 먼저 등록한 항목의 메모나 완료 표시를 덮어쓰지 않습니다.

같은 본문을 다시 분석하면 저장해 둔 결과를 보여 주고 모델을 부르지 않습니다. 이때 답변에 `분석 결과 재사용` 표시가 붙습니다. **본문이 바뀌었으면 언제나 다시 분석합니다** — 내용이 바뀐 페이지에 지난 분석을 붙이지 않기 위해서입니다.

### 일정 탭

| 기능 | 설명 |
|---|---|
| 보기 전환 | 월·주·일 달력과 D-day 목록. 마지막 보기 방식을 기억합니다 |
| 월 격자 | 어느 달이든 6주 42칸 고정. 지난 기한은 붉게, 완료는 취소선 |
| 기한 미정 | 달력 칸에 넣지 않고 건수 띠로 남겨 목록 보기로 연결합니다 |
| 직접 편집 | 추가·수정·삭제(확인 후), 완료 체크, 근거 문장·제출물·문의처 펼쳐보기 |
| 내보내기 | CSV(근거 문장 포함·수식 주입 방지·BOM)와 ICS(`TZID=Asia/Seoul`) |

기한 알림은 **사이드패널을 닫아도** 동작합니다. `chrome.alarms`가 서비스 워커를 깨워 하루 한 번, 지난 기한·오늘·내일을 묶어 알립니다. 알릴 시각은 설정에서 바꾸거나 알림 자체를 끌 수 있습니다. 브라우저가 실행 중이어야 합니다.

### `@일정` — 말로 하는 일정 관리

`@일정 금요일까지 보고서 제출`, `@일정 이번 주 보여줘`, `@일정 교육 신청 지워줘`처럼 문장으로 지시합니다.

- **모델은 분류만 합니다.** 사용자가 읽는 목록과 답변 문장은 코드가 저장소를 직접 읽어 만듭니다. 대화 모델에게 일정 목록을 맡기면 없는 일정을 지어내고, 그것을 구분할 방법이 없기 때문입니다.
- **등록·수정·삭제는 예외 없이 확인 카드를 거칩니다.** 자동 저장 경로는 없습니다.
- **대상을 추측하지 않습니다.** 제목이 여러 건에 걸리면 전부 제시하고 사용자가 고릅니다. 범위 삭제는 “전부”로 지시해도 기본 체크하지 않습니다.
- 날짜는 코드가 직접 읽습니다(`오늘`·`내일`·`이번 주`·`N월 N일`·`N월부터 M월까지` 등). 모델이 좁혀 놓은 조회 범위도 이 값으로 보정합니다.
- 결과 안의 항목 제목이 곧 일정 탭의 그 항목으로 가는 링크입니다. 지운 항목에는 링크를 걸지 않습니다.
