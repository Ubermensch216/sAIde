...
| Node.js / npm | 이번 검증 환경은 Node **24.18.0**, npm **11.16.0**입니다. `package.json`에 최소 버전은 아직 선언되어 있지 않습니다. |
...
현재 도구 실행의 타임아웃과 취소 전달은 보완이 필요합니다. **중단을 눌렀다고 이미 보낸 클릭·입력이 되돌려지는 것은 아닙니다.** 승인 대기 중 페이지가 바뀌었다면 거부하고 다시 시작하세요. 실행 기록의 성공은 DOM 호출 성공이며 웹서비스 저장·전송 완료까지 보증하지 않습니다.
...
보관 기간은 7·30·90일 또는 무기한입니다. 기간 정리는 기억이 켜진 상태로 패널이 열릴 때 실행되며 실시간 만료가 아닙니다. 제외 도메인은 `example.com`처럼 입력합니다. 신규 저장은 하위 도메인도 제외하지만, **기존 기록 삭제는 현재 정확히 일치하는 호스트만 처리하는 결함**이 있습니다.

기억을 끄는 것과 기존 기억 삭제는 별개입니다. 제외·삭제 직전에 이미 진행 중인 저장이 있다면 데이터가 다시 생길 수 있습니다. 기존 기록을 확실히 없애야 하는 경우 기억을 끄고 모든 sAIde 패널을 닫은 뒤 설정에서 **기억 전체 삭제**를 실행하고 저장 개수를 다시 확인하세요.
...
도구 실행 기록에는 읽은 본문 일부 등이 포함될 수 있습니다. 기억 기능을 껐다고 대화나 에이전트 기록까지 저장되지 않는 것은 아닙니다. 별도 앱 수준 암호화·백업/복원·내보내기 UI는 아직 없습니다. 개별 대화 삭제에는 현재 확인/되돌리기가 없습니다. 설정의 사이트 접근 권한 영역에서 권한을 철회할 수 있으며 **권한 철회는 저장된 데이터 삭제와 다릅니다.**
