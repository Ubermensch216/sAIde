...
대화 로드·첨부·생성은 각각 작업 세대로 결과 반영 권한을 검사한다. 임시 응답 ID는 요청별로 다르다. 메시지 저장과 부모 대화 존재 확인은 같은 트랜잭션이다. 설정 저장은 Web Locks로 확장 문서 간 직렬화하고 storage.onChanged로 동기화한다. JSON 기본 15초/embed 180초, 일반 스트림 180초 무수신 제한을 적용한다.
